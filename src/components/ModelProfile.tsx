...
export default function ModelProfile() {
  ...
  const handleFileUpload = async (files: FileList, type: 'photo' | 'video') => {
    ...
    const currentFiles = type === 'photo' ? user?.fotos?.length || 0 : user?.videos?.length || 0;
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const filePath = `${type === 'photo' ? 'photos' : 'videos'}/${fileName}`;
      ...
      const currentFiles = user?.[updateField] || [];
      
      const { error: updateError } = await supabase
        .from('users')
        .update({
          [updateField]: [...currentFiles, ...newFiles]
        })
        .eq('id', user?.id);
      ...
      toast.error(`Erro ao adicionar ${type === 'photo' ? 'fotos' : 'vídeos'}`);
    } finally {
      setIsUploading(false);
    }
  };

  const handleFileDelete = async (url: string, type: 'photo' | 'video') => {
    try {
      // Delete from storage
  ...
  return (
    <div className="min-h-screen bg-luxury-950">
      {/* Profile Header */}
      <div className="bg-luxury-900/50 border-b border-luxury-800">
      ...
      {/* Navigation Tabs */}
      <div className="bg-luxury-900/30 border-b border-luxury-800">
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        {activeTab === 'fotos' && (
          <div className="space-y-6">
            ...
            <PhotoGallery
              photos={user?.fotos || []}
              onDelete={(url) => handleFileDelete(url, 'photo')}
                ...
                return (
                  <div key={index} className="relative aspect-video rounded-lg overflow-hidden group">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-luxury-950 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => handleFileDelete(video, 'video')}
            ...
            <div className="card-luxury divide-y divide-luxury-800">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-luxury-100">Sair</span>
                </div>
                <ChevronRight className="w-5 h-5 text-luxury-400" />
              </button>
            </div>
